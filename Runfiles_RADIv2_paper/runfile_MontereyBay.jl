...
include("modules/Params_MontereyBay.jl");
...
include("setup/IC_MontereyBay.jl");
...
model_params=calculate_constants(T, U, P, Fpom, Fcalcite, 0.8, 0.2, 18.0, 0.05)
...
const model_params_list = Vector{Any}(undef, trajectories)
const param_meta_list = Vector{NamedTuple}(undef, trajectories) 

for i in 1:trajectories
    new_T = 4.0 + rand() * (8.0 - 4.0)
    new_U = 0.05 + rand() * (0.10 - 0.05)
    new_omegaCa = 2.5 + rand() * (5.0 - 2.5)
    new_P = input_P(new_omegaCa, new_T)
    new_Fpom = 70.0 + rand() * (140.0 - 70.0)
    new_Fcalcite = 0.2 + rand() * (0.8 - 0.2)


    new_Fpom_f =  0.5160882157582884#rand() * (0.95 - 0.5) + 0.5
    new_Fpom_s = 1 - new_Fpom_f
    new_kfast =  18.35323727274738#rand() * (30.0 - 2.5) + 2.5
    new_kslow = 0.06952294299846568#rand() * (1e-1 - 1e-4) + 1e-4

    model_params_list[i] = calculate_constants(new_T, new_U, new_P, new_Fpom, new_Fcalcite, new_Fpom_s, new_Fpom_f, new_kfast, new_kslow)
    ...
    param_meta_list[i] = (
        ...
        P = new_P,
        Fpom = new_Fpom,
        Fcalcite = new_Fcalcite,
    )
end

@everywhere function prob_func(prob, i, repeat)
    # Use precomputed values (model_params_list and param_meta_list)
    new_model_params = model_params_list[i]
    pmeta = param_meta_list[i]

    new_T = pmeta.T
    new_U = pmeta.U
    new_P = pmeta.P
    new_Fpom = pmeta.Fpom
    new_Fcalcite = pmeta.Fcalcite

    println("Trajectory $i")

    z = new_model_params.z
...
ensemble_prob = EnsembleProblem(prob, prob_func=prob_func, output_func = (sol, prob) -> (sol[end], false))
...
function adjust_tol_condition(u, t, integrator)
    ...
    return t > 0.5 && !max_tolerances_reached  # Adjust condition as needed
end
...
function progress_callback(integrator)
    ...
    if progress >= next_progress
        println("Progress: $(round(progress * 100))%")
        next_progress += progress_step
    end
end
# Add the progress callback to the integrator
callback = CallbackSet(DiscreteCallback(adjust_tol_condition, adjust_tol_affect!), PeriodicCallback(progress_callback, 1.0))


batch_size = 2  # You can experiment with this
num_batches = ceil(Int, trajectories / batch_size)

all_sols = []

@time begin
    for batch_idx in 1:num_batches
        start_traj = (batch_idx - 1) * batch_size + 1
        end_traj = min(batch_idx * batch_size, trajectories)
        this_batch_size = end_traj - start_traj + 1

        println("Running batch $batch_idx: trajectories $start_traj to $end_traj")

        # Define the local start index
        local_start_idx = start_traj - 1

        # Define a local prob_func customized to this batch
        local_prob_func = function (prob, i, repeat)
            global_i = local_start_idx + i
            new_model_params = model_params_list[global_i]
            pmeta = param_meta_list[global_i]
            ...
            println("Trajectory $global_i (batch local i=$i)")
        ...
        # Create local EnsembleProblem for this batch
        local_prob = EnsembleProblem(prob, prob_func=local_prob_func, output_func=(sol, prob) -> (sol[end], false))
        ...
        append!(all_sols, batch_sols)

        GC.gc()  # Force garbage collection between batches
    end
end
...
@save "MontereyBay_12_05_2025.jld2" param_meta_list all_sols model_params_list
# Save the results
