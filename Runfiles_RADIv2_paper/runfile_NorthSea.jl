...
include("modules/Params_NorthSea.jl");
...
for i in 1:trajectories
    new_T = 5.0 + rand() * (13.0 - 5.0)
    new_U = 0.02 + rand() * (0.10 - 0.02)
    new_omegaCa = 2.5 + rand() * (4.5 - 2.5)
    new_P = input_P(new_omegaCa, new_T)
    new_Fpom = 80.0 + rand() * (170.0 - 80.0)
    new_Fcalcite = 0.4 + rand() * (1.0 - 0.4)


    new_Fpom_f =  0.5193129985657517#rand() * (0.95 - 0.5) + 0.5
    new_Fpom_s = 1 - new_Fpom_f
    new_kfast =  22.95942927210087 #rand() * (30.0 - 2.5) + 2.5
    new_kslow = 0.012110120523478339#rand() * (1e-1 - 1e-4) + 1e-4
...
@save "NorthSea_12_05_2025.jld2" param_meta_list all_sols model_params_list
